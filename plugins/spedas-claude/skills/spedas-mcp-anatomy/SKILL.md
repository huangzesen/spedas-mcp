---
name: spedas-mcp-anatomy
description: >
  The maintenance + navigation convention for the spedas_mcp repo, adapting the
  ...
  Reach for this skill when:
    - You (a coding agent) are about to read or change spedas_mcp code and want
...
# spedas_mcp anatomy — the convention & maintenance contract

The spedas_mcp repo is mapped by a tree of `ANATOMY.md` files (root + `src/spedas_mcp/`, `analysis/`, `datasources/`). This skill is the **convention**; those files are the **content**. It is the top-level maintenance guide for any coding agent touching this repo.
...
`uv run --extra dev --extra mcp --extra analysis python -m pytest -q` (full suite) · `uv run --extra mcp python scripts/smoke_mcp_list_tools.py` (advertised surface) · `python -c "import spedas_mcp.<module>"` (import smoke). If you changed the tool surface, confirm the count before/after.
