...
def _get_cache_root() -> Path:
    ...
    from spedas_mcp.backends.pds.config import get_cache_root
    return get_cache_root()


def _get_missions_dir():
    """Return the bundled missions directory."""
    from spedas_mcp.backends.pds.catalog import get_missions_dir
...
def _fetch_metadata_from_label(dataset_id: str) -> dict | None:
    ...
    from spedas_mcp.backends.pds.metadata import _fetch_metadata_from_label as _impl
...
def refresh_metadata(
    ...
    """
    from spedas_mcp.backends.pds.metadata import get_cache_dir, _dataset_id_to_cache_filename
    ...
    if dataset_ids:
        ...
        from spedas_mcp.backends.pds.catalog import match_dataset_to_mission
        ...
        if not ids_to_refresh:
            from spedas_mcp.backends.pds.catalog import load_mission_json
            try:
...
def refresh_time_ranges(mission: str | None = None) -> dict:
    ...
    """
    from spedas_mcp.backends.pds.scripts.build_catalog import (
...
def rebuild_catalog(mission: str | None = None) -> dict:
    ...
    """
    from spedas_mcp.backends.pds.scripts.build_catalog import build_catalog as _build
...
def build_metadata(
    ...
    from spedas_mcp.backends.pds.metadata import (
