...
These functions implement the first two SPEDAS MCP Phase-2 spectral tools:
...
Design contract (mirrors :mod:`spedas_mcp.analysis.coords`, roadmap epic #5/#7):
...
def _error(
    ...
    **extra: Any,
    ...
    Mirrors :func:`spedas_mcp.analysis.coords._error` and the server's
...
def dynamic_power_spectrum(
    ...
    paths plus compact ranges/shape only. Requires ``spedas-mcp[analysis]``.
...
def wavelet_transform(
    ...
    try:
        ...
        return _error(
            ...
            "Install it with: pip install 'spedas-mcp[analysis]'. "
            f"(import error: {exc})",
